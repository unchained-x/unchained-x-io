/**
 * Forked from three/examples/jsm/tsl/display/BloomNode.js (r183)
 * Change: _nMips reduced from 5 to 4 (saves 2 GPU submissions per frame)
 */
...
class BloomNode extends TempNode {
	...
	constructor( inputNode, strength = 1, radius = 0, threshold = 0 ) {
		...
		this._nMips = 2; // Changed from 5 to 2

		this._renderTargetBright = new RenderTarget( 1, 1, { depthBuffer: false, type: HalfFloatType } );
		this._renderTargetBright.texture.name = 'BloomPass.bright';
		...
		for ( let i = 0; i < this._nMips; i ++ ) {
			...
			renderTargetHorizontal.texture.name = 'BloomPass.h' + i;
	...
	setup( builder ) {
		...
		// gaussian blur materials (4 mips instead of 5)
		const kernelSizeArray = [ 6, 10 ];

		for ( let i = 0; i < this._nMips; i ++ ) {
			this._separableBlurMaterials.push( this._getSeparableBlurMaterial( builder, kernelSizeArray[ i ] ) );
		}

		// composite material (4 levels instead of 5)
		const bloomFactors = uniformArray( [ 1.0, 0.8 ] );
		const bloomTintColors = uniformArray( [ new Vector3( 1, 1, 1 ), new Vector3( 1, 1, 1 ) ] );
